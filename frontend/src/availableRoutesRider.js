          ...
          <tbody>
            {available_trips.map(item => (
              <tr key={item.id}>
                ...
                <td>
                  <button className="table-button" onClick={() => window.location.href=`/map?origin=${item.origin}&destination=${item.destination}&driverInfo=${item.driver_username}&departs=${item.date_time}`}>
                    View
                  </button>
                </td>
                <td> 
